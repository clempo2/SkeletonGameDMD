...
class ScoresLayer(ScriptlessLayer):
    ...
    def regenerate(self):
        ...
        self.script = []
        hsd = self.game.get_highscore_data()
        entry_ct = len(hsd)
        for rec in hsd:
    ...
    def get_duration(self):
        hsd = self.game.get_highscore_data()
        duration = len(hsd)*self.duration
        return duration
...
class ZoomingLayer(Layer):
    ...
    def __init__(self, layer_to_zoom, hold=False, frames_per_zoom = 1, scale_start = 1.0, scale_stop = 2.0, total_zooms=30):
        ...
        self.horz_center = None
        self.vert_center = None

        # self.horz_center = layer_to_zoom.target_x
        # self.vert_center = layer_to_zoom.target_y
    ...
    def next_frame(self):
        ...
        if(self.total_zoomed < self.total_zooms):
            ...
            if self.horz_center != None and self.vert_center != None:
                 # logging.getLogger('zoom_layer').info("adjust position scale current" + str(self.scale_current))
                 # logging.getLogger('zoom_layer').info(self.target_x, self.target_y)
                 new_x = (1-self.scale_current) * self.horz_center
                 new_y = (1-self.scale_current) * self.vert_center
